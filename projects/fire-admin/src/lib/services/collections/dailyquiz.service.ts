...
export class DailyQuizService {
  ...
  private pipeQuizzes(quizzesObservable: Observable<Quiz[]>) {
    return quizzesObservable.pipe(mergeMap(async (quizzes: Quiz[]) => {
      
      //pages.forEach((page: Page) => { // forEach loop doesn't seems to work well with async/await
      for (let quiz of quizzes) {
        console.log(quiz);
        // console.log(page.translations);
        quiz.author = quiz.createdBy ? this.users.getFullName(quiz.createdBy) : of(null);
      }
      //});
      return quizzes;
    }));
  ...
  getWhereFn(queryFn: QueryFn, applyPipe: boolean = false) {
    const quizzesObservable = this.db.getCollection('quizes', queryFn);
    return applyPipe ? this.pipeQuizzes(quizzesObservable) : quizzesObservable;
  }
